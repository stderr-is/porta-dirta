...
# ── 1. Create: Fetch Hotel Context (HTTP POST - FINAL FIX) ───────────────────
# Adds the 'token' to the JSON body as required by the PHP proxy.
hotel_fetch_node = {
    ...
    "parameters": {
        ...
        "sendBody": True,
        "specifyBody": "json",
        "jsonBody": "={{ JSON.stringify({ date: $json.checkIn || new Date().toISOString().split('T')[0], token: $env.BEDS24_API_TOKEN }) }}",
...
# ── 2. Modify: Build Draft Prompt (FINAL FIX) ──────────────────────────────
# Escapes the apostrophe in "Porta D'irta" to prevent SyntaxError.
for node in nodes:
...
let restoNote = 'No hay datos de disponibilidad de restaurante.';
...
let hotelNote = 'No hay datos de disponibilidad de hotel.';
...
"
  + "3. TONO: Cálido, mediterráneo, sofisticado pero cercano.
"
...
print("✅ Final bugfix patch ready.")
